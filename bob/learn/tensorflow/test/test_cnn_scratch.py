...
def scratch_network_embeding_example(train_data_shuffler,
                                     reuse=False,
                                     get_embedding=False):
...
def validate_network(embedding,
                     validation_data,
                     validation_labels,
                     input_shape=[None, 28, 28, 1],
                     normalizer=scale_factor):
    # Testing
    validation_data_shuffler = Memory(
        validation_data,
        validation_labels,
    ...
    accuracy = 100. * numpy.sum(
        numpy.argmax(predictions, axis=1) == labels) / predictions.shape[0]
...
def test_cnn_tfrecord():
    ...
    filename_queue = tf.train.string_input_producer(
        [tfrecords_filename], num_epochs=15, name="input")
    ...
    filename_queue_val = tf.train.string_input_producer(
        [tfrecords_filename_val], num_epochs=15, name="input_validation")
    ...
    loss = mean_cross_entropy_loss(logits, labels)
    validation_loss = mean_cross_entropy_loss(validation_logits,
                                              validation_labels)
    ...
    saver.restore(session,
                  tf.train.latest_checkpoint(
                      os.path.dirname("./temp/cnn_scratch/")))
...
def test_cnn_tfrecord_embedding_validation():
    ...
    labels = train_data_shuffler("label", from_queue=False)
    validation_logits = scratch_network_embeding_example(
