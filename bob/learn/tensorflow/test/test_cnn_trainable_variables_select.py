...
def base_network(train_data_shuffler,
                 reuse=False,
                 get_embedding=False,
                 trainable=True):
...
def test_trainable_variables():
    ...
    filename_queue = tf.train.string_input_producer(
        [tfrecords_filename], num_epochs=1, name="input")
    ...
    conv1_after_first_train = tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES,
        scope='conv1')[0].eval(session=trainer.session)[0]
    ...
    embedding = base_network(
        train_data_shuffler, get_embedding=True, trainable=False)
    ...
    trainer.load_variables_from_external_model(
        step1_path, var_list=['conv1', 'fc1'])

    conv1_restored = tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES,
        scope='conv1')[0].eval(session=trainer.session)[0]
    ...
    trainer.train()
    conv1_after_second_train = tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES,
        scope='conv1')[0].eval(session=trainer.session)[0]
