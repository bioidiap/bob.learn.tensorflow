...
class Conv1D(Layer):
    ...
    **Parameters**
    ...
    def __init__(self,
                 ...
                 filters=20,
                 stride=100,
                 weights_initialization=tf.contrib.layers.xavier_initializer(
                     uniform=False, dtype=tf.float32, seed=10),
                 init_value=None,
                 bias_initialization=tf.contrib.layers.xavier_initializer(
                     uniform=False, dtype=tf.float32, seed=10),
                 batch_norm=False,
                 use_gpu=False):
    ...
    def create_variables(self, input_layer):
        ...
        if len(input_layer.get_shape().as_list()) != 3:
            raise ValueError(
                "The input as a convolutional layer must have 3 dimensions, "
                "but {0} were provided".format(
                    len(input_layer.get_shape().as_list())))
        n_channels = input_layer.get_shape().as_list()[2]

        if self.W is None:
            if self.init_value is None:
                self.init_value = self.kernel_size * n_channels
            self.W = self.weights_initialization(
    ...
    def get_graph(self, training_phase=True):
        ...
        with tf.name_scope(str(self.name)):
            conv1d = tf.nn.conv1d(
                self.input_layer, self.W, stride=self.stride, padding='VALID')
